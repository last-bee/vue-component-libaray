<description>
### sessionStorage
封装了浏览器的 `sessionStorage` api，使得本地存储的操作更加简便和规范。
</description>
...
export default {
  methods: {
    setData: function () {
      ...
      // 如果是全局公共数据请传递common
      var prefixName = 'sessionStorageDemo'
      var name = 'testName' // 数据名
      var value = 'testValue' // 数据值
      this.$sessionStorage.set(prefixName, name, value)
      Toast.success('存储成功')
    },
    getData: function () {
      var prefixName = 'sessionStorageDemo'
      var name = 'testName' // 数据名
      Toast('获取到的值为：' + this.$sessionStorage.get(prefixName, name))
    },
    removeData: function () {
      var prefixName = 'sessionStorageDemo'
      var name = 'testName' // 数据名
      this.$sessionStorage.remove(prefixName, name)
      Toast.success('移除完成')
    },
    clearData: function () {
      var prefixName = 'sessionStorageDemo' // 清除该前缀的所有数据
      this.$sessionStorage.clear(prefixName, name)
